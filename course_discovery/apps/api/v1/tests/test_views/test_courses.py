...
class CourseViewSetTests(OAuth2Mixin, SerializationMixin, APITestCase):
    ...
    def test_unsubmitted_status(self):
        ...
        assert {c['uuid'] for c in response.data['results']} == {str(run.course.uuid) for run in [future_end_run, no_end_run]}
    ...
    def test_cannot_change_type_after_review(self):
        ...
        assert response.status_code == 400
        assert response.data == ('Switching entitlement types after being reviewed is not supported. Please reach out to your ' + 'project coordinator for additional help if necessary.')
    ...
    def test_options(self):
        ...
        assert data['level_type']['choices'] == [{'display_name': self.course.level_type.name_t, 'value': self.course.level_type.name_t}]
        assert data['entitlements']['child']['children']['mode']['choices'] == [{'display_name': 'Audit', 'value': 'audit'}, {'display_name': 'Credit', 'value': 'credit'}, {'display_name': 'Honor', 'value': 'honor'}, {'display_name': 'Professional', 'value': 'professional'}, {'display_name': 'Verified', 'value': 'verified'}]
