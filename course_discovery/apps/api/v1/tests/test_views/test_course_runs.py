...
class CourseRunViewSetTests(SerializationMixin, ElasticsearchTestMixin, OAuth2Mixin, APITestCase):
    ...
    def test_create_succeeds_with_failed_image_update_to_studio(self):
        ...
        assert mock_logger.call_args_list[0] == mock.call('An error occurred while setting the course run image for [{key}] in studio. All other fields were successfully saved in Studio.'.format(key=new_key))
    ...
    def test_partial_update_no_studio_url(self):
        ...
        assert mock.call('Not pushing course run info for %s to Studio as partner %s has no studio_url set.', self.draft_course_run.key, self.partner.short_code) in mock_logger.call_args_list

        assert mock.call('Not updating course run image for %s to Studio as partner %s has no studio_url set.', self.draft_course_run.key, self.partner.short_code) in mock_logger.call_args_list
    ...
    def test_options(self):
        ...
        assert data['level_type']['choices'] == [{'display_name': self.course_run.level_type.name_t, 'value': self.course_run.level_type.name_t}, {'display_name': self.course_run_2.level_type.name_t, 'value': self.course_run_2.level_type.name_t}, {'display_name': self.draft_course_run.level_type.name_t, 'value': self.draft_course_run.level_type.name_t}]
        assert data['content_language']['choices'] == [{'display_name': x.name, 'value': x.code} for x in LanguageTag.objects.all().order_by('name')]
