...
class UserAttributeTests(TestCase):
    def test_str(self):
        ...
        assert str(user_attr) == '{user}: {enable_email_notification}'.format(user=user_attr.user, enable_email_notification=user_attr.enable_email_notification)
...
class OrganizationUserRoleTests(TestCase):
    ...
    def test_str(self):
        ...
        assert str(self.org_user_role) == '{organization}: {user}: {role}'.format(organization=self.org_user_role.organization, user=self.org_user_role.user, role=self.org_user_role.role)
