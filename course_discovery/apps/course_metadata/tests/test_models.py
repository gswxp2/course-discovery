...
class TestCourse(TestCase):
    ...
    def test_html_fields_are_validated(self, field_name):
        ...
        with pytest.raises(ValidationError):
            course.clean_fields()
        assert cm.exception.message_dict[field_name] == ['Invalid HTML received']
...
class CourseRunTests(OAuth2Mixin, TestCase):
    ...
    def test_enrollable_seats(self):
        ...
        assert course_run.enrollable_seats([verified_seat_type, professional_seat_type]) == [verified_seat, professional_seat]
...
class SubjectTests(SiteMixin, TestCase):
    ...
    def test_validate_unique(self):
        ...
        assert str(validation_error.exception) == "{'name': ['Subject with this Name and Partner already exists']}"
...
class TopicTests(SiteMixin, TestCase):
    ...
    def test_validate_unique(self):
        ...
        assert str(validation_error.exception) == "{'name': ['Topic with this Name and Partner already exists']}"
...
class CourseUrlSlugHistoryTest(TestCase):
    ...
    def test_slug_with_partner_mismatch(self):
        ...
        with pytest.raises(ValidationError):
            slug_object.save()
        assert validation_error.exception.message_dict['partner'] == ['Partner {partner_key} and course partner {course_partner_key} do not match when attempting to save url slug {url_slug}'.format(partner_key=mismatch_partner.name, course_partner_key=slug_object.course.partner.name, url_slug=slug_object.url_slug)]
