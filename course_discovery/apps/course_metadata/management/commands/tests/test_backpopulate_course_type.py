...
class BackpopulateCourseTypeCommandTests(TestCase):
    ...
    def test_invalid_args(self):
        ...
        assert cm.exception.args[0] == 'No courses found. Did you specify an argument?'

        with pytest.raises(CommandError) as cm:
            self.call_command(course_arg)  # no partner
        assert cm.exception.args[0] == 'You must specify --partner'
        ...
        assert cm.exception.args[0] == 'Supplied Course Type slug [NotACourseType] does not exist.'

        with pytest.raises(CommandError) as cm:
            self.call_command('--allow-for=verified-audit:NotACourseRunType', partner_code, course_arg)
        assert cm.exception.args[0] == 'Supplied Course Run Type slug [NotACourseRunType] does not exist.'
    ...
    def test_args_from_database(self):
        ...
        assert args['partner'] == 'a'
        assert args['commit'] is True
        assert args['course'] == ['b', 'c']
        assert args['org'] == ['d', 'e']
    ...
    def test_existing_type(self):
        ...
        self.run_command()
        assert self.course.type == prof_course_type
        assert self.course.modified == pre_course_modified
        assert self.audit_run.type == audit_run_type
        assert self.audit_run.modified == pre_audit_modified
        assert self.verified_run.type == audit_run_type
        assert self.verified_run.modified == pre_verified_modified
    ...
    def test_matches_earliest_course_type(self):
        ...
        self.run_command()
        assert self.course.type == second_type
    ...
    def test_by_org(self):
        ...
        assert self.course.type.empty
        assert self.course2.type == self.audit_course_type
