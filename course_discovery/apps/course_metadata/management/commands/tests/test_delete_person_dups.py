...
class DeletePersonDupsCommandTests(TestCase):
    ...
    def test_invalid_args(self):
        ...
        assert cm.exception.args[0] == 'You must specify at least one person'

        with pytest.raises(CommandError) as cm:
            self.call_command(uuid_arg)  # no partner
        assert cm.exception.args[0] == 'You must specify --partner-code'
        ...
        assert cm.exception.args[0] == 'Malformed argument "a", should be in form of UUID:TARGET_UUID'

        with pytest.raises(CommandError) as cm:
            self.call_command(partner_code, 'a:a')
        assert cm.exception.args[0] == 'Malformed argument "a:a", UUIDs cannot be equal'
